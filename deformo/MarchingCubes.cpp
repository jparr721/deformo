...
void MarchingCubes::GenerateGeometry(MatrixXr& V, MatrixX<int>& F,
    ...
    for (unsigned int z = 0; z < n_cells_z_; ++z) {
        for (unsigned int y = 0; y < n_cells_y_; ++y) {
            for (unsigned int x = 0; x < n_cells_x_; ++x) {
                ...
                if (scalar_field_[z * points_in_slice + y * points_in_x_dir +
                                  x] < iso_level_) {
                    table_index |= 1;
                }

                if (scalar_field_[z * points_in_slice + (y + 1) * points_in_x_dir +
                                  x] < iso_level_) {
                    table_index |= 2;
                }

                if (scalar_field_[z * points_in_slice +
                                  (y + 1) * points_in_x_dir + (x + 1)] <
                    iso_level_) {
                    table_index |= 4;
                }
                ...
                if (scalar_field_[(z + 1) * points_in_slice +
                                  y * points_in_x_dir + x] < iso_level_) {
                    table_index |= 16;
                }

                if (scalar_field_[(z + 1) * points_in_slice +
                                  (y + 1) * points_in_x_dir + x] < iso_level_) {
                    table_index |= 32;
                }
                ...
                if (scalar_field_[(z + 1) * points_in_slice +
                                  y * points_in_x_dir + (x + 1)] < iso_level_) {
                    table_index |= 128;
                }
                ...
                if (edge_table.at(table_index) != 0) {
                    ...
                    for (unsigned int i = 0;
                        ...
                        triangle.x() = EdgeId(
                            x, y, z, triangle_table.at(table_index).at(i));
                        triangle.y() = EdgeId(
                            x, y, z, triangle_table.at(table_index).at(i + 1));
                        triangle.z() = EdgeId(
                            x, y, z, triangle_table.at(table_index).at(i + 2));
...
inline auto MarchingCubes::Interpolate(const Real x1, const Real y1,
                                       const Real z1, const Real x2,
                                       const Real y2, const Real z2,
                                       const Real val1, const Real val2)
    -> Vector3r {
...
inline auto MarchingCubes::Intersection(const unsigned int x,
    ...
    const unsigned int points_in_slice =
        points_in_x_direction * (n_cells_y_ + 1);
    const Real val1 = scalar_field_[v1z * points_in_slice +
                                    v1y * points_in_x_direction + v1x];
    const Real val2 = scalar_field_[v2z * points_in_slice +
                                    v2y * points_in_x_direction + v2x];
    return Interpolate(x1, y1, z1, x2, y2, z2, val1, val2);
}

inline auto MarchingCubes::VertexId(const unsigned int x, const unsigned int y,
                                    const unsigned int z) -> unsigned int {
    return 3 *
           (z * (n_cells_y_ + 1) * (n_cells_x_ + 1) + y * (n_cells_x_ + 1) + x);
}

inline auto MarchingCubes::EdgeId(const unsigned int x, const unsigned int y,
                                  const unsigned int z,
                                  const unsigned int edge_number)
    -> unsigned int {
    switch (edge_number) {
...
inline void MarchingCubes::CalculateFacesAndVertices(
    MatrixXr& V, MatrixX<int>& F, std::vector<Vector3<int>>& triangles,
    std::map<unsigned int, Position>& vertices) {
