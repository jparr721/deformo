...
class tensor {
  public:
    ...
    /*
    Creates a tensor with the eigen matrix type
    */
    template <typename Derived>
    explicit tensor(const Eigen::MatrixBase<Derived>& values);

    /*
    Creates a tensor with the deformo tensor type
    */
    template <typename T> explicit tensor(const Tensor3<T>& values);
     ...
     */
    template <typename T> tensor(const std::initializer_list<T>& values);
     ...
     */
    template <typename T> tensor(const T& value);
...
inline tensor::tensor(enum TF_DataType type, const void* data, size_t len,
                      const std::vector<int64_t>& shape) {
    this->tf_tensor = {TF_AllocateTensor(type, shape.data(), shape.size(), len),
                       TF_DeleteTensor};
...
template <typename Derived>
inline tensor::tensor(const Eigen::MatrixBase<Derived>& values)
    : tensor(std::vector<Derived::Scalar>(
                 values.data(), values.data() + values.rows() * values.cols()),
             {values.rows(), values.cols()}) {}

template <typename T>
inline tensor::tensor(const Tensor3<T>& values)
    : tensor(deduce_tf_type<T>(), values.Instance().data(),
             values.Dimensions().prod() * sizeof(T),
             {values.Dimension(0), values.Dimension(1), values.Dimension(2)}) {}
...
template <> inline tensor::tensor(const std::string& value) {
    TF_TString tstr[1];
...
template <> inline tensor::tensor(const std::string& value) {
    ...
    *this =
        tensor(static_cast<enum TF_DataType>(TF_STRING), (void*)data, size, {});
    delete[] data;
}
...
template <typename T> std::vector<T> tensor::get_data() const {
...
inline std::shared_ptr<TF_Tensor> tensor::get_tensor() const {
    if (!tf_tensor) {
        tf_tensor = {
            TFE_TensorHandleResolve(tfe_handle.get(), context::get_status()),
            TF_DeleteTensor};
        status_check(context::get_status());
    }
    return tf_tensor;
